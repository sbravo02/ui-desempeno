...
export default function EvaluacionWizardPage(){
  ...
  const step = stepParam === 2 ? 2 : 1; // 1=Pedagógica (Likert), 2=Materia (MCQ)

  const questions = step === 1 ? PEDAGOGIC : SUBJECT(subject);

  // Respuestas: para likert guardamos 1..5 (número); para mcq guardamos string de la opción elegida
  ...
  function setLikert(qid: string, n: number){ setAnswers(a => ({ ...a, [qid]: n })); }
  function setMcq(qid: string, opt: string){ setAnswers(a => ({ ...a, [qid]: opt })); }
  ...
  return (
    <main className="max-w-2xl mx-auto p-6 space-y-4">
      <div>
        ...
        <p className="text-sm text-zinc-600">Docente: {teacher} · Materia: {subject}</p>
      </div>
              ...
              <li key={q.id}>
                ...
                {q.kind === "mcq" && (
                  <div className="grid grid-cols-2 gap-2">
                    {(q as McqQ).options.map(opt => (
                      <label key={opt} className={`flex items-center gap-2 rounded-xl border p-2 cursor-pointer ${answers[q.id] === opt ? "border-zinc-900" : ""}`}>
                        <input
                          type="radio"
                          name={q.id}
                          value={opt}
