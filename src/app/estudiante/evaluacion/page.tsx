...
function SUBJECT(subject: string): McqQ[] {
  ...
  return [{ id: "s-1", kind: "mcq", text: `Pregunta de opción múltiple para ${subject}`, options: ["Opción A", "Opción B", "Opción C", "Opción D"] }];
}
...
function EvaluacionWizardInner() {
  ...
  const canSubmit = useMemo(() => {
    ...
    return questions.every(q => {
      ...
      if ((q as LikertQ).kind === "likert") return typeof v === "number" && v >= 1 && v <= 5;
                ...
                {(q as any).kind === "likert" ? (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(q as McqQ).options.map(opt => (
                      <label
                        key={opt}
                        className={`flex items-center gap-2 rounded-xl border p-2 cursor-pointer ${answers[q.id] === opt ? "border-zinc-900" : ""}`}
                      >
                        <input
                          type="radio"
                          name={q.id}
